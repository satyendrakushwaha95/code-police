    ...
    switch (options.format) {
      ...
      case 'pdf': {
        ...
        const pdfPath = options.outputPath
          || path.join(os.tmpdir(), `codepolice-report-${options.scanId}${FORMAT_EXT.pdf}`);
...
<footer>
  Generated by Code Police &middot; ${this.esc(new Date().toISOString())}
</footer>
      ...
      runs: [
        {
          tool: {
            driver: {
              name: 'Code Police',
              version: '1.0.0',
              informationUri: 'https://github.com/satyendrakushwaha95/code-police',
              rules: rules.map(r => {
  ...
  private generateMarkdown(data: ReportData, options: ReportOptions): string {
    ...
    lines.push(`---`);
    lines.push(`*Generated by Code Police — ${new Date().toISOString()}*`);
